...
class Router
{
    public function start()
    {
        ...
        if ($params[0] != '') {
            ...
            // Récupère la méthode ou utilise 'index' par défaut
            $action = isset($params[0]) ? array_shift($params) : 'index';

            if (method_exists($controller, $action)) {
                // Exécute la méthode avec ou sans paramètres
                $controller->$action(...$params);
            } else {
                // Retourne une erreur 404 si la méthode n'existe pas
                http_response_code(404);
                echo 'Page introuvable';
            }
        } else {
            // Charge le contrôleur et la méthode par défaut
